...
function showShowtimes() {
    ...
    // Nếu không có giờ chiếu, ẩn phần chọn giờ chiếu
    if (showtimes.length === 0) {
        showtimesSection.style.display = "none";
        alert("Không có giờ chiếu cho rạp này.");
        return;
    }
...
function redirectToLogin() {
    ...
    if (selectedTime) {
        window.location.href = "login.html";  // Đảm bảo đường dẫn chính xác
    } else {
        alert("Vui lòng chọn giờ chiếu trước khi đăng nhập!");
    }
}
